Feature: Reporting out of memory events
    ...
    #   And the payload field "events.0.exceptions.0.errorClass" equals "foo"
    #   And the payload field "events.0.unhandled" is false
    #   And the payload field "events.0.severity" equals "warning"
    #   And the payload field "events.1.exceptions.0.errorClass" equals "Out Of Memory"
    #   And the payload field "events.1.exceptions.0.message" equals "The app was likely terminated by the operating system while in the foreground"
    #   And the payload field "events.1.session.events.handled" equals 1
    #   And the payload field "events.1.session.events.unhandled" equals 1
    ...
    #   And the payload field "events.1.exceptions.0.errorClass" equals "Out Of Memory"
    #   And the payload field "events.1.exceptions.0.message" equals "The app was likely terminated by the operating system while in the foreground"
    #   And the payload field "events.1.session" is null
