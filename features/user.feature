...
Scenario: User fields set as null
    When I run "UserDisabledScenario" with the defaults on "iPhone 8"
...
Scenario: Only User email field set
    When I run "UserEmailScenario" with the defaults on "iPhone 8"
    Then I should receive a request
    And the request is a valid for the error reporting API
...
Scenario: All user fields set
    When I run "UserEnabledScenario" with the defaults on "iPhone 8"
    Then I should receive a request
    And the request is a valid for the error reporting API
    And the "Bugsnag-API-Key" header equals "a35a2a72bd230ac0aa0f52715bbdc6aa"
...
Scenario: Only User ID field set
    When I run "UserIdScenario" with the defaults on "iPhone 8"
    Then I should receive a request
    And the request is a valid for the error reporting API
